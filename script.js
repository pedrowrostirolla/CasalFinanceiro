...
async function handleAuth() {
    ...
    if (isLoginMode) {
        ...
        alert("Usuário cadastrado! Agora faça o login.");
        toggleAuth();
    }
}
...
function toggleAuth() {
    ...
    document.getElementById('auth-toggle').innerText = isLoginMode ? "Primeiro acesso? Cadastre-se" : "Já tem conta? Entrar";
}
...
async function deleteArr(id) {
    if(confirm("Excluir esta fonte de arrecadação?")) {
...
// --- ATUALIZAÇÃO DE DADOS E DASHBOARD ---
async function loadAppData() {
    ...
    // Atualiza Tabela de Arrecadação
    const tableArr = document.querySelector('#table-arr tbody');
    tableArr.innerHTML = '';
    
    arrs.forEach(a => {
        select.innerHTML += `<option value="${a.nome}">${a.nome}</option>`;
        tableArr.innerHTML += `<tr><td>${a.nome}</td><td>R$ ${a.valor.toFixed(2)}</td><td><button onclick="deleteArr(${a.id})">❌</button></td></tr>`;
    });
...
async function updateDashboard() {
    ...
    movs.sort((a,b) => new Date(b.data) - new Date(a.data)).forEach(m => {
        ...
        tableBody.innerHTML += `<tr>
            ...
            <td><button onclick="deleteMov(${m.id})">❌</button></td>
        </tr>`;
    });
